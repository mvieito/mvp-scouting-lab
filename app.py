...
def process_data(df):
    ...
    # Limpieza estricta de decimales
    cols_to_int = ['Age', 'Edad', 'Min', 'Minutos', 'Matches', 'Partidos', 'Partidos jugados', 'MP']
...
num_cols = [c for c in df.select_dtypes(include=[np.number]).columns if c not in ['Age', 'Edad', 'Born', 'Matches', 'Min', 'ID', 'Partidos jugados', 'MP']]

# --- FILTROS LIMPIOS (Vacíos por defecto) ---
st.sidebar.markdown("---")
...
if 'Pos' in df.columns or 'Posición específica' in df.columns:
    col_pos = 'Pos' if 'Pos' in df.columns else 'Posición específica'
    posiciones = st.sidebar.multiselect("Posición", sorted(df[col_pos].unique()), default=[])
    if posiciones: mask &= df[col_pos].isin(posiciones)
...
def get_pos_group(p):
    ...
    else: return 'MF' # Por defecto (Medios y pivotes)

col_pos_valida = 'Pos' if 'Pos' in df_f.columns else ('Posición específica' if 'Posición específica' in df_f.columns else None)
if col_pos_valida:
    df_f['Grupo_Pos'] = df_f[col_pos_valida].apply(get_pos_group)
else:
    df_f['Grupo_Pos'] = 'MF' # Fallback si no hay columna de posiciones
...
# Configuración visual de columnas (Anti-decimales)
col_config = {
...
# --- TAB 1: RANKING ---
with t1:
    # Mostramos las columnas base y el score final. (No mostramos las métricas específicas aquí porque varían por jugador)
    base_cols = [c for c in ['Player', 'Squad', 'Competition', 'Age', 'Pos', 'Min'] if c in df_f.columns] + ['Grupo_Pos', 'Score']
...
with t3:
    if players:
        ...
        g_pos = d['Grupo_Pos']
        mets_jugador = config_pos[g_pos]['metrics'] # Rescatamos las métricas con las que fue evaluado
        ...
        st.markdown(f'''
            <div style="background: #0f172a; padding: 25px; border-radius: 12px; border-left: 8px solid #fbbf24; margin-bottom: 20px; display:flex; justify-content: space-between;">
                <div><h1 style="color: white; margin:0;">{d['Player']}</h1><h4 style="color: #94a3b8; margin:0;">{d.get('Squad','-')} | {g_pos}</h4></div>
        ...
        # Mostramos las 3 métricas con las que fue evaluado
    ...
    if not df_f.empty:
        ...
        with c_pitch:
            ...
            if pitch_data:
                ...
                # Fichas tácticas (Visibles al 100%)
                ...
                # Nombres de jugadores con FONDO OSCURO para que no se pierdan en el césped
                for _, row in df_pitch.iterrows():
                    fig.add_annotation(
                        x=row['x'], y=row['y']-6,
                        text=f"<b>{row['Name'].split(' ')[-1]}</b>", # Solo el apellido
                        showarrow=False,
                        font=dict(color="white", size=12, family="Inter"),
                        bgcolor="#0f172a", # Fondo oscuro sólido
                        bordercolor="#fbbf24",
                        borderwidth=1,
                        borderpad=3,
                        opacity=0.95
                ...
                fig.update_layout(xaxis=dict(visible=False, range=[0,100]), yaxis=dict(visible=False, range=[0,100]),
                                  height=600, margin=dict(l=0,r=0,t=0,b=0), paper_bgcolor="rgba(0,0,0,0)", showlegend=False)
                st.plotly_chart(fig, use_container_width=True)
